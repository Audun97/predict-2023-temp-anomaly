...
plt.plot(merged_data['Year'], results.fittedvalues, color='orange', linestyle='-', label='Fitted line')

residuals = merged_data['Anomaly_Full_Year'] - results.fittedvalues
residuals_year = pd.concat([merged_data['Year'], residuals], axis=1)
residuals_year.columns = ['Year', 'Residuals']
residuals_year.to_csv(r'D:\Projects\predict_2023_temp\residuals.csv', index=False)
plt.show()